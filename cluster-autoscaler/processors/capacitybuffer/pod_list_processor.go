...
func (p *CapacityBufferPodListProcessor) Process(ctx *context.AutoscalingContext, unschedulablePods []*apiv1.Pod) ([]*apiv1.Pod, error) {
	...
	_, toBeProvisionedBuffers := p.toProvisionFilter.Filter(buffers)

	totalFakePods := []*apiv1.Pod{}
	for _, buffer := range toBeProvisionedBuffers {
		fakePods := p.provision(buffer)
		totalFakePods = append(totalFakePods, fakePods...)
	}
	klog.V(2).Infof("Capacity pod processor injecting %v fake pods provisioning %v capacity buffers", len(totalFakePods), len(toBeProvisionedBuffers))
...
func (p *CapacityBufferPodListProcessor) provision(buffer *api_v1.CapacityBuffer) []*apiv1.Pod {
	...
	podTemplate, err := p.client.GetPodTemplate(podTemplateName)
	if err != nil {
...
func makeFakePods(bufferId types.UID, samplePodTemplate *apiv1.PodTemplateSpec, podCount int) ([]*apiv1.Pod, error) {
	var fakePods []*apiv1.Pod
	samplePod := getPodFromTemplate(samplePodTemplate)

	for i := 1; i <= podCount; i++ {
		newPod := withCapacityBufferFakePodAnnotation(samplePod)
		newPod.Name = fmt.Sprintf("capacity-buffer-%s-%d", bufferId, i)
		newPod.UID = types.UID(fmt.Sprintf("%s-%d", string(bufferId), i))
