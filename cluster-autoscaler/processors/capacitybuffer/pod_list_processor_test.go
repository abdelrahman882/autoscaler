...
func TestPodListProcessor(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			objectsInKubernetesClient:    []runtime.Object{},
			objectsInBuffersClient:       []runtime.Object{getTestingBuffer("b", "ref", 1, 1, false)},
		...
		},
		{
			...
			objectsInKubernetesClient:    []runtime.Object{},
			objectsInBuffersClient:       []runtime.Object{getTestingBuffer("buffer", "ref", 1, 1, true)},
			unschedulablePods:            []*corev1.Pod{getTestingPod("Pod")},
			expectedUnschedPodsCount:     1,
			expectedUnschedFakePodsCount: 0,
		...
		},
		{
			...
			objectsInKubernetesClient:    []runtime.Object{getTestingPodTemplate("ref")},
			objectsInBuffersClient:       []runtime.Object{getTestingBuffer("buffer", "ref", 1, 1, true)},
			unschedulablePods:            []*corev1.Pod{getTestingPod("Pod")},
		...
		},
		{
			...
			objectsInKubernetesClient:    []runtime.Object{getTestingPodTemplate("ref1"), getTestingPodTemplate("ref2")},
			objectsInBuffersClient:       []runtime.Object{getTestingBuffer("buffer1", "ref1", 3, 1, true), getTestingBuffer("buffer2", "ref2", 5, 1, true)},
		...
		},
		{
			name:                         "Mixing cases",
			objectsInKubernetesClient:    []runtime.Object{getTestingPodTemplate("ref1"), getTestingPodTemplate("ref2")},
			objectsInBuffersClient:       []runtime.Object{getTestingBuffer("buffer1", "ref1", 3, 1, false), getTestingBuffer("buffer2", "ref2", 5, 1, true)},
			unschedulablePods:            []*corev1.Pod{getTestingPod("Pod1"), getTestingPod("Pod2"), getTestingPod("Pod3")},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			fakeCapacityBuffersClient, _ := client.NewCapacityBufferClientFromClients(fakeBuffersClient, fakeKubernetesClient)

			processor := NewCapacityBufferPodListProcessor(fakeCapacityBuffersClient)
			...
			for buffer, condition := range test.expectedBuffersProvCondition {
				buffer, err := fakeBuffersClient.AutoscalingV1().CapacityBuffers(corev1.NamespaceDefault).Get(context.TODO(), buffer, metav1.GetOptions{})
...
func getTestingPodTemplate(name string) *corev1.PodTemplate {
	return &corev1.PodTemplate{ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: "default"}}
}

func getTestingBuffer(bufferName, refName string, replicas int32, generation int64, ready bool) *apiv1.CapacityBuffer {
	buffer := &apiv1.CapacityBuffer{ObjectMeta: metav1.ObjectMeta{Name: bufferName, Namespace: "default"}}
	buffer.Status = *testutil.GetBufferStatus(&apiv1.LocalObjectRef{Name: refName}, &replicas, &generation, nil)
	if ready {
