...
func (f *combinedFilter) Filter(buffers []*v1.CapacityBuffer) ([]*v1.CapacityBuffer, []*v1.CapacityBuffer) {
	var totalToBeProcessed []*v1.CapacityBuffer
	for _, buffersFilter := range f.filters {
		toBeProcessed, filteredOutBuffers := buffersFilter.Filter(buffers)
		buffers = filteredOutBuffers
		totalToBeProcessed = append(totalToBeProcessed, toBeProcessed...)
	}
	return totalToBeProcessed, buffers
}
