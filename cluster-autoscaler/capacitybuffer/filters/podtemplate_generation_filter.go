...
// podTemplateGenerationFilter filters out buffers with the defined conditions
type podTemplateGenerationFilter struct {
	client *cbclient.CapacityBufferClient
}

// NewPodTemplateGenerationFilter creates an instance of podTemplateGenerationFilter that filters the buffers that needs to be updated.
func NewPodTemplateGenerationFilter(client *cbclient.CapacityBufferClient) *podTemplateGenerationFilter {
	return &podTemplateGenerationFilter{
		client: client,
	}
}

// Filter filters the passed buffers based on buffer status conditions
func (f *podTemplateGenerationFilter) Filter(buffersToFilter []*v1.CapacityBuffer) ([]*v1.CapacityBuffer, []*v1.CapacityBuffer) {
...
func (f *podTemplateGenerationFilter) podTemplateGenerationChanged(buffer *v1.CapacityBuffer) bool {
	if buffer.Status.PodTemplateRef == nil || buffer.Status.PodTemplateGeneration == nil {
		return false
	}

	podTemplate, err := f.client.GetPodTemplate(buffer.Status.PodTemplateRef.Name)
...
func (f *podTemplateGenerationFilter) CleanUp() {
}
