...
func (t *podTemplateBufferTranslator) Translate(buffers []*v1.CapacityBuffer) []error {
	...
	for _, buffer := range buffers {
		if isPodTemplateBasedBuffer(buffer) {
			...
			podTemplate, err := t.client.GetPodTemplate(podTemplateRef.Name)
			if err != nil {
			...
			if numberOfPods == nil {
				common.SetBufferAsNotReadyForProvisioning(buffer, podTemplateRef, &podTemplate.Generation, nil, fmt.Errorf("Failed to get buffer's number of pods"))
				continue
			}
			common.SetBufferAsReadyForProvisioning(buffer, podTemplateRef, &podTemplate.Generation, numberOfPods)
		}
	}
	return errors
}
