...
func NewPodTemplateBufferTranslator(client kubernetes.Interface) *podTemplateBufferTranslator {
...
func (t *podTemplateBufferTranslator) Translate(buffers []*v1.CapacityBuffer) []error {
	...
	for _, buffer := range buffers {
		if isPodTemplateBasedBuffer(buffer) {
			...
			podTemplateRef, numberOfPods := t.translate(buffer)
			podTemplate, err := common.GetPodTemplate(t.client, podTemplateRef.Name)

			if err != nil {
				common.SetBufferAsNotReadyForProvisioning(buffer, err.Error())
			...
			if numberOfPods == nil {
				buffer.Status.PodTemplateRef = &v1.LocalObjectRef{
					Name: podTemplateRef.Name,
				}
				buffer.Status.Replicas = nil
				buffer.Status.PodTemplateGeneration = &podTemplate.Generation
			} else {
				common.SetBufferAsReadyForProvisioning(buffer, podTemplateRef.Name, podTemplate.Generation, *numberOfPods)
			}
...
func (t *podTemplateBufferTranslator) translate(buffer *v1.CapacityBuffer) (*v1.LocalObjectRef, *int32) {
	if buffer.Spec.Replicas != nil {
		replicas := max(0, int32(*buffer.Spec.Replicas))
		return buffer.Spec.PodTemplateRef, &replicas
	}
