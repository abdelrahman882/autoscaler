...
func (t *resourceLimitsTranslator) Translate(buffers []*api_v1.CapacityBuffer) []error {
	...
	for _, buffer := range buffers {
		if isResourcesLimitsDefinedInBuffer(buffer) {
			...
			podTemplate, err := t.client.GetPodTemplate(buffer.Status.PodTemplateRef.Name)
			if err != nil {
			...
			common.SetBufferAsReadyForProvisioning(buffer, buffer.Status.PodTemplateRef, &podTemplate.Generation, numberOfPods)
		}
	}
	return errors
}
