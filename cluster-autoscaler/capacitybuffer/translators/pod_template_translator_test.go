...
func TestPodTemplateBufferTranslator(t *testing.T) {
	...
	fakeCapacityBuffersClient, _ := cbclient.NewCapacityBufferClient(nil, fakeClient, nil, nil, nil, nil, nil, nil, nil)
	tests := []struct {
		name                   string
	...
	}{
		{
			...
			expectedStatus: []*v1.CapacityBufferStatus{
				testutil.GetBufferStatus(&v1.LocalObjectRef{Name: registeredPodTemplate.Name}, &testutil.SomeNumberOfReplicas, &registeredPodTemplate.Generation, testutil.GetConditionReady()),
			},
		...
		},
		{
			...
			expectedStatus: []*v1.CapacityBufferStatus{
				testutil.GetBufferStatus(&v1.LocalObjectRef{Name: registeredPodTemplate.Name}, &testutil.SomeNumberOfReplicas, &registeredPodTemplate.Generation, testutil.GetConditionReady()),
				testutil.GetBufferStatus(&v1.LocalObjectRef{Name: anotherRegisteredPodTemplate.Name}, &testutil.AnotherNumberOfReplicas, &anotherRegisteredPodTemplate.Generation, testutil.GetConditionReady()),
			},
			expectedNumberOfErrors: 0,
		},
		{
			...
			expectedStatus: []*v1.CapacityBufferStatus{
				testutil.GetBufferStatus(&v1.LocalObjectRef{Name: registeredPodTemplate.Name}, &testutil.SomeNumberOfReplicas, &registeredPodTemplate.Generation, testutil.GetConditionReady()),
				testutil.GetBufferStatus(nil, nil, nil, testutil.GetConditionNotReady()),
			},
		...
		},
		{
			...
			expectedStatus: []*v1.CapacityBufferStatus{
				testutil.GetBufferStatus(&v1.LocalObjectRef{Name: registeredPodTemplate.Name}, &testutil.SomeNumberOfReplicas, &registeredPodTemplate.Generation, testutil.GetConditionReady()),
				testutil.GetBufferStatus(&v1.LocalObjectRef{Name: anotherRegisteredPodTemplate.Name}, nil, &anotherRegisteredPodTemplate.Generation, testutil.GetConditionNotReady()),
			},
			expectedNumberOfErrors: 0,
		},
		{
