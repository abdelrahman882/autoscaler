...
func TestPodTemplateBufferTranslator(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedStatus: []*v1.CapacityBufferStatus{
				testutil.GetBufferStatus(&v1.LocalObjectRef{Name: registeredPodTemplate.Name}, &testutil.SomeNumberOfReplicas, &registeredPodTemplate.Generation, testutil.GetConditionReady()),
				testutil.GetBufferStatus(&v1.LocalObjectRef{Name: anotherRegisteredPodTemplate.Name}, nil, &anotherRegisteredPodTemplate.Generation, nil),
			},
			expectedNumberOfErrors: 0,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			podTemplateBufferTranslator := NewPodTemplateBufferTranslator(fakeClient)
